        ...
        if hasattr(node, 'body') and isinstance(node.body, list):
            for ind, x in enumerate(node.body):
                if not imports_inserted:
                    if is_import(x):
                        # TODO: is itt ok tto import everytthing?
                        # for type in mutation_types:
                        #     import_node = generate_import_node(type)
                        #     node.body.insert(ind+num_ins_imps, mutatation_node)
                        #     num_ins_imps+=1
            ...
            for ind, x in enumerate(node.body):
                ...
                if is_specific_call(x, 'fit'):
                    ...
                    if hasattr(x.value, 'args') and len(x.value.args) > 0:
                        ...
                        # Old versions
                        # print(type(x.value.args[0]))
                        # keywords.append(ast.keyword(arg="x_train_len", value=ast.Call(func=ast.Name(id='len', ctx=ast.Load()),
                        #                                             args=[ast.Name(id=x.value.args[0].id, ctx=ast.Load()),], keywords=[])))
                        # keywords.append(
                        #     ast.keyword(arg="x", value=ast.Name(id=x.value.args[0].id,ctx=ast.Load())))
                        # keywords.append(
                        #     ast.keyword(arg="x", value=x.value.args[0]))
...
# def is_activation_def_by_arg(elem):
#     # Type A
#     res = False
#     type = None
#     pos = None
...
#             and hasattr(elem.value.args[0], 'args') and len(elem.value.args[0].args) > 1 \
#             and isinstance(elem.value.args[0].args[1], ast.Str) \
#             and elem.value.args[0].args[1].s in const.activation_functions:
#         res = True
#         type = 'A'
#
#     return res, type, pos
...
def rename_trained_model(file_path):
    ...
    new_file_name = file_path[0] + file_path[2]
